# Youtube Link for Lab-8:- https://youtu.be/FtZKD1FHhjM 
...
- High Availability: If one MongoDB pod fails, the other two continue serving requests. No downtime for the pet store application.

- Data Persistence: Each pod has its own PersistentVolume. If a pod restarts or is deleted, the data survives because it's stored on the persistent disk, not inside the container.

- Replica Set: Provides automatic failover. If the primary node fails, one of the secondary nodes automatically becomes primary. Data is synchronized across all three replicas.



### rabbitmq

- Message Persistence: Messages in queues are stored on persistent disk. If the RabbitMQ pod crashes or restarts, messages aren't lost. Critical for order processing no customer orders disappear.

- Queue Durability: Queue configurations (exchanges, bindings, user settings) survive pod restarts. No need to reconfigure RabbitMQ after every restart.

- Data Location: RabbitMQ stores all its data in /var/lib/rabbitmq By mounting a PVC here, everything persists.

- Reliability: Orders from customers are safely queued even during pod failures, ensuring the makeline service can process them when it's ready.
...
**Purpose:** Fully managed, globally distributed NoSQL database with MongoDB API compatibility.

**Advantages:**
- **99.999% availability** 
- **Automatic scaling** 
- **Automatic backups** - Every 4 hours with point-in-time restore (30 days)
- **Global distribution** 
- **Zero maintenance** 
- **Built-in security** - Encryption, compliance certifications 
...
#### **Azure Service Bus**
**Purpose:** Fully managed enterprise message broker for reliable queuing and pub/sub messaging.
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: order-service
          image: venom0836/order-service-l8:latest # Custom image for order service
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: makeline-service
          image: venom0836/makeline-service-l8:latest # Custom Makeline service image
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: product-service
          image: venom0836/product-service-l8:latest # Custom product service image
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: store-front
          image: venom0836/store-front-l8:latest # Front-end store application
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: store-admin
          image: venom0836/store-admin-l8:latest # Admin interface for store
          ports:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: ai-service
          image: venom0836/ai-service-l8:latest
          ports:
            - containerPort: 5001
          env:
...
spec:
  serviceName: mongodb # Associated headless service for DNS resolution
  replicas: 3 # Number of MongoDB replicas
  selector:
    matchLabels:
      app: mongodb # Labels to match for this StatefulSet
  template:
  ...
  volumeClaimTemplates:
    - metadata:
      ...
      spec:
        ...
        resources:
          requests:
            storage: 1Gi   # size of each replica’s volume
        storageClassName: managed-csi # Storage class for persistent volume 
---
# Service to expose MongoDB
apiVersion: v1
kind: Service
        ...
        - name: rabbitmq
          ...
          volumeMounts: # Mount configuration for RabbitMQ plugins
            - name: rabbitmq-enabled-plugins
              ...
              subPath: enabled_plugins
            - name: rabbitmq-data
              mountPath: /var/lib/rabbitmq # Data storage path
      volumes:
        - name: rabbitmq-enabled-plugins
          configMap:
            name: rabbitmq-enabled-plugins
